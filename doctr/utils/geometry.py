...
def polygon_to_bbox(polygon: Polygon4P) -> BoundingBox:
    ...
    Returns:
        a bounding box
    """
    # Avoid zip(*polygon) to reduce temporary tuple/list allocations
    # Directly compute min/max in a single pass for better performance
    it = iter(polygon)
    try:
        x0, y0 = next(it)
    except StopIteration:
        # polygon is empty; let .min()/.max() raise ValueError as in original impl
        return (min([]), min([])), (max([]), max([]))  # triggers ValueError
    min_x = max_x = x0
    min_y = max_y = y0
    for x, y in it:
        if x < min_x:
