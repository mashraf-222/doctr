...
def remove_image_padding(image: np.ndarray) -> np.ndarray:
    ...
    # Optimize axis reductions by combining them into a single operation for better cache locality
    # and process both axes in one pass.
    # We avoid extra function calls and object allocations.

    # Collapse channel (if present) for non-zero check
    if image.ndim == 3:
        projection = np.any(image, axis=2)
    else:
        projection = image != 0

    rows_any = np.any(projection, axis=1)
    cols_any = np.any(projection, axis=0)

    row_indices = np.flatnonzero(rows_any)
    col_indices = np.flatnonzero(cols_any)
